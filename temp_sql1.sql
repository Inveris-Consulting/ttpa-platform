  ...
  WITH 
    ...
    unified_facts AS (
      ...
      SELECT 
        ...
        d."Hour" AS hour_num,
        COALESCE(r."Representative", CASE WHEN ct."WFE" IS TRUE THEN 'WFE' ELSE 'Unassigned' END) AS rep_name,
        COALESCE(ct."Form Name", 'Unassigned Form') AS form_name,
        COALESCE(ct."Source", 'Unassigned') AS source,
        0 AS duration_in_call, 0 AS duration_total, true AS answered
      FROM public."fDeals" d
...
BEGIN
  ...
  WITH 
    ...
    all_submissions AS (
      SELECT s.*, COALESCE(r."Representative", CASE WHEN ct."WFE" IS TRUE THEN 'WFE' ELSE NULL END) AS ttpa_rep_name, ct."Source", ct."Form Name" AS crm_form_name
      FROM public."fSubmissions" s
